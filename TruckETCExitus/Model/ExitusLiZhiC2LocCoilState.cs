...
{
    class ExitusLiZhiC2LocCoilState : ExitusState
    {
                ...
                && csUnit.Buffer[csUnit.Buffer.Length - 1] == Antenna.eof[0] && csUnit.Buffer.Length >= 6)
            {
                switch (csUnit.Buffer[Antenna.CMD_LOC])
                {
                    ...
                    case 0xD1:
                        ...
                        {
                            if (preLoc != null && preLoc.POstate == 0)//定位有效
                            {
                                preLoc.POstate = 1;
                                if (preLoc.XOBUCoord > -(50 + 300 / 2 + 70) && preLoc.XOBUCoord < (50 + 300 / 2))//横坐标范围
                                {
                                    if (Global.coils.CoilStatus[0].StatNow == Coil.CurStatus.Sheltered)
                                    {
                                        if (preLoc.YOBUCoord > -50 && preLoc.YOBUCoord < 700 + 100)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (preLoc.YOBUCoord > -50 && preLoc.YOBUCoord < 450 + 100)
                                            {
                                                HandleD1Frame(csUnit, rtb);
                                                break;
                                            }
                        ...
                        //UpdatePreAntannaMsg(csUnit, "D1到来,发送C2", Color.Red, rtb);
                        ResetPreStep();
                        break;
                    case 0xD2:
                        if (preLoc != null && preLoc.POstate == 0)//定位有效
        ...
        protected virtual void HandleD1Frame(CSUnit csUnit, RichTextBox rtb)
        {
            ...
            {
                UpdatePreAntannaMsg(csUnit, "D1到来,含有obu号,发送C2", Color.Red, rtb);
                Global.PreAntenna.Send(Antenna.C2Frame);
                return;
            }
                ...
                switch (csUnit.Buffer[Antenna.CMD_LOC])
                {
                    ...
                    case 0xB2:
                        ...
                        else
                        {
                            ...
                            UpdateTrdAntannaMsg(csUnit, "B2帧错误到来", Color.Red, rtb);
                            ResetTrdStep();
                        }
                        break;
                    case 0xB3:
        ...
        private void HandleC2Frame(CSUnit csUnit, RichTextBox rtb)
        {
            ...
            {                
                ...
                UpdateLocSrvMsg(csUnit, "收到上位机预读C2", Color.Red, rtb);
                Global.PreAntenna.Send(Antenna.C2Frame);
                ResetPreStep();
            }
            else
            {
                ...
                {
                    UpdateLocSrvMsg(csUnit, "收到上位机交易C2", Color.Red, rtb);
                    Global.TrdAntenna.Send(Antenna.C2Frame);
                    ResetTrdStep();
                }
                else
